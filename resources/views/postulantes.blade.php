...
<style>
    ...
    .btn-action { padding: 4px 8px; font-size: 12px; border-radius: 4px; text-decoration: none; display: inline-block;}
    .btn-view { background: #5c6bc0; color: white; }
    ...
    .status-observado { background-color: #e0f2fe; color: #0369a1; border-color: #7dd3fc; }
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="primera_opcion_carrera_id" required>
                            ...
                            @foreach($carreras as $carrera)
                                <option value="{{ $carrera->id }}" {{ old('primera_opcion_carrera_id') == $carrera->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select name="segunda_opcion_carrera_id" required>
                            <option value="">Seleccione...</option>
                            @foreach($carreras as $carrera)
                                <option value="{{ $carrera->id }}" {{ old('segunda_opcion_carrera_id') == $carrera->id ? 'selected' : '' }}>
                                    {{ $carrera->codigo_carrera }} - {{ $carrera->nombre }}
                                </option>
                            @endforeach
                        </select>
    ...
    @if(isset($modo) && $modo === 'editar')
        <div class="form-panel">
            ...
            <form action="{{ route('postulantes.update', $postulante) }}" method="POST">
                ...
                <div style="background: #f8fafc; padding: 12px; border-radius: 6px; margin-bottom: 15px; font-size: 14px;">
                    <span style="margin-right: 20px;"><strong>Username:</strong> {{ $postulante->user->username }}</span>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        <label>CI:</label>
                        <input type="text" name="ci" class="form-control" value="{{ old('ci', $postulante->user->ci) }}" required>
                    </div>
                    <div class="form-group">
                        <label>Nombre:</label>
                        <input type="text" name="name" class="form-control" value="{{ old('name', $postulante->user->name) }}" required>
                    </div>
                    <div class="form-group">
                        <label>Apellido:</label>
                        <input type="text" name="apellido" class="form-control" value="{{ old('apellido', $postulante->user->apellido) }}" required>
                    </div>
                    <div class="form-group">
                        <label>Teléfono:</label>
                        <input type="text" name="telefono" class="form-control" value="{{ old('telefono', $postulante->user->telefono) }}">
                    </div>
                    <div class="form-group">
                        <label>Correo:</label>
                        <input type="email" name="email" class="form-control" value="{{ old('email', $postulante->user->email) }}" required>
                    </div>
                </div>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select name="genero">
                            ...
                            <option value="otro" {{ old('genero', $postulante->genero) === 'otro' ? 'selected' : '' }}>Otro</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Dirección:</label>
                    ...
                    <div class="form-group">
                        ...
                        <select name="grupo_id">
                            ...
                            @foreach($grupos as $grupo)
                                <option value="{{ $grupo->id }}" {{ old('grupo_id', $postulante->grupo_id) == $grupo->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select name="estado_inscripcion" required>
                            <option value="pendiente" {{ old('estado_inscripcion', $postulante->estado_inscripcion) === 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                            <option value="observado" {{ old('estado_inscripcion', $postulante->estado_inscripcion) === 'observado' ? 'selected' : '' }}>Observado</option>
                            <option value="aceptado" {{ old('estado_inscripcion', $postulante->estado_inscripcion) === 'aceptado' ? 'selected' : '' }}>Aceptado</option>
                            <option value="rechazado" {{ old('estado_inscripcion', $postulante->estado_inscripcion) === 'rechazado' ? 'selected' : '' }}>Rechazado</option>
                        </select>
                    </div>
    ...
    @if(isset($modo) && $modo === 'detalle')
        <div class="form-panel" style="line-height: 1.6;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 10px;">
                ...
                <p><strong>Primera opción:</strong> {{ $postulante->primeraOpcionCarrera->codigo_carrera ?? '' }} - {{ $postulante->primeraOpcionCarrera->nombre ?? 'No registrado' }}</p>
                <p><strong>Segunda opción:</strong> {{ $postulante->segundaOpcionCarrera->codigo_carrera ?? '' }} - {{ $postulante->segundaOpcionCarrera->nombre ?? 'No registrado' }}</p>
                <p><strong>Grupo:</strong> {{ $postulante->grupo ? $postulante->grupo->nombre . ' - ' . $postulante->grupo->gestion : 'Sin grupo asignado' }}</p>
            ...
            <tbody>
                @forelse($postulantes as $item)
                    <tr>
                        ...
                        <td>
                            <small>{{ $item->primeraOpcionCarrera->codigo_carrera ?? '' }} - {{ $item->primeraOpcionCarrera->nombre ?? '' }}</small>
                        </td>
                        <td>
                            <small>{{ $item->segundaOpcionCarrera->codigo_carrera ?? '' }} - {{ $item->segundaOpcionCarrera->nombre ?? '' }}</small>
                        </td>
                        <td>
                            @if($item->grupo)
                                {{ $item->grupo->nombre }}
                        ...
                        <td>
                            <form action="{{ route('postulantes.updateEstado', $item->id) }}" method="POST" style="margin:0;">
                                @csrf
                                @method('PATCH')
                                <select name="estado_inscripcion" 
                                        class="select-status-inline status-{{ $item->estado_inscripcion }}" 
                                        onchange="this.form.submit()">
                                    <option value="pendiente" {{ $item->estado_inscripcion === 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                                    <option value="observado" {{ $item->estado_inscripcion === 'observado' ? 'selected' : '' }}>Observado</option>
                                    <option value="aceptado" {{ $item->estado_inscripcion === 'aceptado' ? 'selected' : '' }}>Aceptado</option>
                                    <option value="rechazado" {{ $item->estado_inscripcion === 'rechazado' ? 'selected' : '' }}>Rechazado</option>
                                </select>
                            </form>
                        </td>
                        ...
                        <td>
                            ...
                            <form action="{{ route('postulantes.destroy', $item) }}" method="POST" style="display:inline; margin:0;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-action btn-delete" style="border:none; cursor:pointer;" onclick="return confirm('¿Seguro que desea eliminar este postulante?')">
                                    Eliminar
                                </button>
                            </form>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="11" style="text-align: center; color: #94a3b8; padding: 20px;">No hay postulantes registrados.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
